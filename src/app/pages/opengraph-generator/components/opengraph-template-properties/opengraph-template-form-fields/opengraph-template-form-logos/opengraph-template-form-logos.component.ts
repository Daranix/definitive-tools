import {
  OpenGraphImage,
  OpenGraphTemplateFormInputFontOptions,
  OpenGraphTemplateLogos,
} from '@/app/pages/opengraph-generator/types';
import {
  Component,
  effect,
  model,
...
export class OpengraphTemplateFormLogosComponent {
  ...
  readonly tagFontOptions = model<OpenGraphTemplateFormInputFontOptions>(
    TAG_DEFAULT_FONT_OPTIONS,
  );
  readonly titleFontOptions = model<OpenGraphTemplateFormInputFontOptions>(
    TITLE_DEFAULT_FONT_OPTIONS,
  );
  readonly tag = model<string>('Easy Storage');
  readonly title = model<string>('Managed Databases for Everyone');

  readonly firstLogo = model<OpenGraphImage | undefined>({
    name: 'logo.png',
    url: '/img/opengraph-examples/logo-1.png',
  });
  readonly secondLogo = model<OpenGraphImage | undefined>({
    name: 'logo.png',
    url: '/img/opengraph-examples/logo-1.png',
  });
  readonly thirdLogo = model<OpenGraphImage | undefined>({
    name: 'logo.png',
    url: '/img/opengraph-examples/logo-1.png',
  });
  ...
  onImageFileChanged(
    logo: 'firstLogo' | 'secondLogo' | 'thirdLogo',
    event?: File,
  ) {
    const data = event
      ? { name: event.name, url: URL.createObjectURL(event) }
      : undefined;
    if (logo === 'firstLogo') {
